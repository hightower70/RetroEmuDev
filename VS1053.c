...
void MP3Init( WORD mode)
{
    ...
    writeMP3Register( MP3_REG_MODE, MP3_MODE_SDINEW | mode); // set native + mode
//  writeMP3Register( MP3_REG_BASS, 0);
    writeMP3Register( MP3_REG_CLOCKF, 0x2000);      // double the frequency 

    // can change to full speed now
    MP3_SPICON1 = 0x007E;

    //for( i = 0; i<12; i++)
    //    array[i] = readMP3Register( i);
